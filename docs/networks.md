...
|---|---|---|---|---|
| Base Sepolia (testnet) | `base_sepolia` | 84532 | `0x036CbD53842c5426634e7929541eC2318f3dCF7e` | ✅ **the testnet we deploy to** (#1) |
...
| Base | `base` | 8453 | `0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913` | ✅ **the mainnet we deploy to** (#1), after the audit |
...
## The chain we settle on: Base (decided)

**Decided, issue #1:** `base_sepolia` (84532) for the testnet deployment, `base` (8453) for production after
the audit. Lowest fees of the candidates, native Circle USDC, and it runs the Cancun opcodes our default
build targets. Rationale and what would reopen it: [`adr/0001-settlement-network.md`](adr/0001-settlement-network.md).

Every other network in the table above is *supported by the contract*, not deployed to. Adding one means its
own float, Safe, partner allowlist, operator key, monitor and review — a business decision, not a config flag.

If you ever do deploy elsewhere, pick a chain that your **NGN off-ramp partner** and your **custody provider**
both support, with native USDC. Lower fees matter because every transfer is at least two transactions
(`lockQuote`, `settle`).

### EVM version

`foundry.toml` builds with `evm_version = "cancun"`, which makes solc emit `MCOPY`. Base supports it. A chain
that has not activated Cancun rejects that opcode, and the vault either fails to deploy or deploys and then
reverts in use — so `make preflight` probes the target chain for `MCOPY` before you spend gas (issue #25).
If a chain fails that check, build, deploy **and verify** with `make build-shanghai` / `FOUNDRY_PROFILE=shanghai`;
that profile produces different bytecode from the default.
...
network to use (issue #1, now decided as Base), ask in the same message: **"Which token, and on which chain,
does the off-ramp partner settle in?"** — that answer is still outstanding and is the one thing that could
reopen the chain decision.
