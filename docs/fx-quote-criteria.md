...
| 9 | Provider divergence triggers an internal alert | ✅ | Deviation (rounded **up**) ≥ 1% emits `RateDivergence`; > 5% blocks the lock; a missing or stale reference **blocks the lock** (`ReferenceRateUnavailable`) unless admin has set `allowStaleReferenceRate`, in which case it emits `ReferenceRateStale`. Only registered currencies can be quoted, so the check cannot be skipped with an unknown code. `monitor/` alerts on these events **and** on reverted vault transactions, so blocked quotes (`RateDivergenceTooHigh`) raise a critical alert | `test_divergence_*`, `testFuzz_divergence_behaviour`, `test_staleReference_blocksTheLockByDefault`, `test_referenceAgeExactlyMax_isFresh`, `test_unknownCurrency_cannotBypassDivergence`, `script/e2e-local.sh` | 🔗 Ops routes the webhook to Slack/PagerDuty |

## Definition of Done

| Item | Settlement layer status | Evidence |
|---|---|---|
| Functional tests passed | ✅ | `forge test`: 151 unit and fuzz tests plus 8 invariants, 100% line and branch coverage of `src/` |
...
6. Run the rate oracle **before** locking anything: with `allowStaleReferenceRate` false (the default), a
   missing or stale reference rate makes every `lockQuote` revert.
7. If the on-ramp partner delivers USDC per transfer, have them call `fund(ref, usdcAmount + feeUsdc)` before
